...
class RNAFolder(ABC, RNAStructure, StructureIO, StructureConvert):
    ...
    def _compute_h_and_J(self):
        ...
        h = {
            ...
            - 2
            * self.config.args.equality_constraint_fraction
            * self.config.args.equality_constraint_constant
            * self.n
        ...
        for i in range(self.len_stem_list):
            for j in range(i + 1, self.len_stem_list):
                ...
                if is_pseudo:
                    J[(i, j)] += (
                        self.config.args.pseudo_factor
                        * 2
                        * self.config.args.coeff_max_bond
                        * self.stems[i][2]
                        * self.stems[j][2]
                    )
