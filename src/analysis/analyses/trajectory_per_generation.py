...
class TrajectoryPerGeneration(Analysis):
    ...
    def _analyze(self):
        ...
        self.energy_diff = [
            self._calc_energy_diff(self.config.data["energies"][k], self.config.data["energies"][k+self.config.data["generation_size"]]) for k in range(0, len(self.config.data["sequences"])-self.config.data["generation_size"], self.config.data["generation_size"])
        ]

        self._generate_output_2D(self.config.args.output, [self.iterations, self.energy_diff])
