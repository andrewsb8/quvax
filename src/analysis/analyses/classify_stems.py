...
class ClassifyStems(Analysis):
	...
	def _analyze(self):
		...
		self.stems = StructureConvert()._connect_table_to_stems(sequence_len, connect_table_df)
		...
		self.pseudos, self.overlaps = self._get_pseudos_and_overlaps(self.stems)
	...
	def _get_pseudos_and_overlaps(self,stems):
		...
		for i in range(self.num_stems):
			for j in range(i+1,self.num_stems):
				...
				if self.rna_struct_obj._is_pseudo(stems[i],stems[j]):
					pseudos += 1			
				if self.rna_struct_obj._detect_stem_overlap(stems[i],stems[j]):
					overlaps += 1		
