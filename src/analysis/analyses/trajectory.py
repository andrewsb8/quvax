...
class Trajectory(Analysis):
    ...
    def _analyze(self):
        ...
        self.energy_diff = [
            self._calc_energy_diff(self.config.data["energies"][dumb], self.config.data["energies"][k+self.config.data["generation_size"]]) for k in range(dumb, len(self.config.data["sequences"])-self.config.data["generation_size"], self.config.data["generation_size"])
        ]
