...
class Database(object):
        ...
        if db_type == "sqlite":
            ...
            if create: #only need to do this if creating a database
                ...
                try:
                    ...
                    conn.rollback()
                    cursor.close()
                    log.info("Database exists in postgres client.")
            # connect to database
            db = psycopg2.connect(
    ...
    def _prepare_db(self, config):
        if config.args.hash_value is None:
            ...
            config.log.info("Job Hash generated by hashlib: " + str(config.args.hash_value))
        try:
            ...
            config.log.info("Created database tables in " + config.args.output + "\n\n")
        except:
            config.db.rollback()
            config.log.info("Adding data to existing tables within database.\n\n")
            config.db_cursor.execute(
    ...
    def _load_db(self, config):
        ...
        """
        config.log.info("Loading info from database " + config.args.input)
        ...
        config.log.info("Loaded info from database " + config.args.input)
    ...
    def _get_sim_details(self, config):
        ...
        config.log.info("Input Optimization Details:")
        for i in range(len(keys)):
            config.log.info(keys[i][0] + " : " + str(sim_details[0][i]))
